...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        # self.model = torch.compile(self.model)  # Disabled: Triton not available on Windows
        ...
        if len(set(all_file_paths)) != len(all_file_paths):
            raise ValueError("There are duplicate sessions listed in the dataset")

        # Split trials into train and test sets using the configured test_percentage
